...
export default function AssistantPage() {
  const [messages, setMessages] = useState<Array<{ role: string; content: string }>>([
    { role: "assistant", content: "Hello! I am Avanta CRM AI. I have full read access to your active deals, accounts, and contact matrices. Ask me to identify high-value targets, write outreach sequences, or summarize recent logs!" }
  ]);
  ...
  return (
    <div className="h-[80vh] flex flex-col gap-6 animate-fade-in">
      ...
      <div className="border-b border-slate-900 pb-4 flex justify-between items-center">
        ...
        <div className="hidden sm:flex items-center gap-1.5 text-xs text-cyan-400 font-bold bg-cyan-500/10 border border-cyan-500/20 px-3 py-1.5 rounded-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0">
        ...
        <div className="hidden lg:flex flex-col gap-4 bg-slate-900/20 border border-slate-900 p-5 rounded-2xl h-full justify-between shadow-neon-accent">
          ...
          <div className="bg-cyan-500/5 border border-cyan-500/10 p-4 rounded-xl text-[10px] text-slate-400 leading-relaxed">
            <AlertCircle size={14} className="text-cyan-400 mb-1.5" />
            <span>Avanta CRM AI utilizes Vector similarity queries (`pgvector`) to read details from accounts and sync tasks dynamically.</span>
          </div>
        </div>
              ...
              return (
                <div key={idx} className={`flex gap-3 max-w-[85%] ${isAi ? "mr-auto" : "ml-auto flex-row-reverse"}`}>
                  <div className={`
                    ...
                    ${isAi ? "bg-cyan-600 shadow-md shadow-cyan-550/10" : "bg-slate-950 border border-slate-800"}
                  `}>
                    {isAi ? <Bot size={16} /> : <User size={16} className="text-cyan-400" />}
                  </div>
                  <div className={`
            ...
            {loading && (
              <div className="flex gap-3 mr-auto items-center">
                ...
                <div className="p-3 bg-slate-950 border border-slate-900 rounded-2xl flex items-center gap-2 text-xs text-slate-500 font-semibold">
                  ...
                  <span>Avanta AI is analyzing CRM datasets...</span>
                </div>
              </div>
            )}
            <div ref={chatEndRef} />
          ...
          <div className="p-4 bg-slate-950 border-t border-slate-900">
            ...
            >
              <input
                ...
                className="flex-grow bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-xs text-slate-100 placeholder-slate-600 px-4 py-3 rounded-xl outline-none transition-all"
                disabled={loading}
              />
              <button
                type="submit"
                disabled={loading || !input.trim()}
                className="px-4 py-3 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition-all shadow-md flex items-center gap-1 cursor-pointer"
              >
                <Send size={14} />
